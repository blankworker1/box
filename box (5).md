...
- **SETTLE** — completion of a transaction: the padlock/keybar handoff for barter (Layer 2), and the SETTLE App's auto-sweep trigger for Bitcoin (Layer 3, §5a) — both physically or cryptographically irreversible the instant they happen, the same finality Bitcoin settlement has on-chain.
...
Each plate's 12-word seed is reused indefinitely across many successive occupants and transactions. The 12 words are treated as **public, not secret** — anyone who has ever opened that locker has seen them, and the design doesn't fight that. All security lives in the two remaining pieces, generated fresh for every single transaction and never reused: a private half and a passphrase. Deriving and settling these is the job of a dedicated tool — **the SETTLE App.**

### The SETTLE App

A static, client-side, offline-capable web app (same pattern as any standalone BIP39 tool — no server, no account, no data leaves the device except what the user deliberately shares) that performs the whole 5a transaction as three data blocks:

| Block | Contents | Origin | Lifespan |
|---|---|---|---|
| **1 — Locker** | 12-word public plate | Fixed, scanned from the locker door | Permanent |
| **2 — Generated** | Fresh private half + computed 24th word (last 3 entropy bits fixed by convention, so the 24th word never needs transmitting on its own) | Fresh, every transaction | Single-use — destroyed after settlement |
| **3 — Passphrase** | Chosen at deposit | Fresh, every transaction | Single-use |

**Depositor flow (arming a locker):**
1. Scan Block 1 (locker's public QR).
2. App generates Block 2.
3. Enter the passphrase (Block 3).
4. App derives the wallet; depositor deposits funds.
5. App displays Block 2 as an on-screen QR, to be shared at handoff.
6. Block 3 sent to the recipient separately.

**Recipient flow — this is SETTLE:**
1. Scan Block 1.
2. Enter/scan the **personal destination wallet address** (arms the tool, does nothing yet).
3. Enter Block 3 (the passphrase).
4. Scan Block 2 — this is the trigger. The app derives the wallet and **immediately auto-sweeps to the address entered in step 2.**
5. A brief on-screen confirmation (derived balance + destination address) displays for a beat before broadcasting — not a blocking approval step, just a check against a mis-scan or a fat-fingered address under an automatic, irreversible trigger.

Front-loading everything non-secret (destination address, passphrase) before the one genuinely ephemeral final piece (Block 2) keeps the window between "wallet exists" and "wallet is emptied" as short as the time it takes to scan and broadcast — the core design goal.

**Implementation status**: the working prototype scans/generates all three blocks correctly (real BIP39 entropy, real SHA-256 checksum, native Web Crypto, no simulated cryptography) and, on the final scan, **compiles and displays the completed phrase for import into an external wallet app** — it does not yet perform the elliptic-curve derivation, signing, or broadcast needed for genuine one-tap auto-sweep. That's a deliberate scope boundary, not an oversight: EC wallet operations deserve dedicated, tested implementation before being trusted with real funds, not a rushed addition. Auto-sweep as described above remains the target design; closing this gap is listed under Open Questions.

**Two modes, and the difference matters — name whichever is used, don't let it default silently:**
- **In-person mode (default)**: Block 2 is shown screen-to-screen at the moment the keybar physically changes hands, never forwarded ahead of time. This preserves the original two-channel guarantee — physical presence *and* a digital message are both required, and Block 1 being public means the keybar handoff is the thing actually gating access.
- **Remote mode**: Block 2 is sent digitally (screenshot, photo) rather than shown in person, for when a party can't reach the truck. This is a deliberate, documented trade-off, not a hidden one — since Block 1 is already public, sending Block 2 digitally too means the transaction no longer structurally requires anyone's physical presence at all. What protects the funds shifts from "you must be there" to "both parties act immediately and nothing leaks in transit" — a behavioural guarantee, not a structural one. All parties should understand this distinction explicitly before choosing remote mode.

**Why the SETTLE App is specifically designed against shoulder-surfing:**
The "always attended, in public" MVP setting makes on-screen entry of a passphrase or a plate's words a real observation risk. The app's design responds directly: front-loading the non-secret steps (address entry) means the *visible* part of the interaction can happen in the open without exposing anything, while the two secret-bearing moments — passphrase entry and the Block 2 scan-to-broadcast — are single, brief actions that can be done with the phone held low and angled away, rather than a longer manual process (reading words aloud, typing a full mnemonic) that gives an observer more time and more to see.

**Why this works — three independent pieces, none of them sufficient alone:**
Block 1 is public by design and carries no weight. Blocks 2 and 3 are both required and travel on genuinely separate channels in the default mode (physical handoff vs. digital message) — an attacker needs both, and neither is reusable afterward since both are single-transaction and destroyed/burned on use.

**Hard rule — a derived wallet is burned the instant it's opened, whether or not funds are found:**
Both the depositor (who generated Block 2 and chose Block 3) and, after settlement, anyone who saw Block 2 in transit, always retain enough to re-derive that specific wallet. The recipient's auto-sweep on scanning Block 2 is what neutralizes this — it is the entire security boundary of this tier, not a best practice layered on top.

**Scope**: a permanent BOX service for low-value amounts among the trust group — not a phase to outgrow. Not suitable for larger amounts or for anyone outside the trust group; larger amounts belong in 5b instead.

### 5b. Personal bearer object — a self-minted, sealed single-sig coin
...
### The double-spend problem — and each tier's different fix
Whoever created the wallet always retains knowledge of the underlying secret, regardless of lock quality, padlock genuineness, or physical settlement. No mechanism at the BOX layer can close this gap entirely — it's the general Bitcoin custody-handoff problem — but the two tiers close it differently, matching their different purposes:

- **5a** is a one-shot settlement rail, not meant to hold value at rest: **the recipient sweeps to their own separate wallet immediately on taking possession.** Once swept, the depositor's knowledge becomes worthless, the same way any emptied wallet's seed is worthless after its coins have moved. This is a protocol rule — a habit — not a hardware feature, and it is 5a's entire security boundary.
- **5b** is meant to circulate sealed, like a coin, not be swept on every handoff — its protection is the **self-mint rule**: if the ceremony is followed correctly, nobody but the current owner ever knew the seed in the first place, so there's no prior holder's knowledge to worry about until the owner themselves chooses to pass the coin on. The risk shifts entirely to whether that rule was actually followed at minting.
...
- Preferred Block 3 (passphrase) transmission channel for remote-mode 5a transfers
- Where the practical line sits between "low-value, use 5a" and "significant enough, bring your own 5b plate"
- **Add real EC wallet derivation, signing, and broadcast to the SETTLE App** — closing the gap between the current compile-and-display prototype and true one-tap auto-sweep; needs a properly vetted Bitcoin library, not a rushed addition
- Whether remote mode should require an explicit acknowledgement step ("I understand this transaction has no physical-presence guarantee") before it can be used
...
- Source the injection-molding process and multi-color waste plastic feedstock for 5b casings; prototype and stress-test tamper-evidence before trusting it with real value
- Define the Nostr note format for a 5b mint record (photo + xpub) and confirm it rides the existing TAZ Tools one-directional sync to the GitHub Node
...
- Write the self-mint ceremony script itself (script/checklist for the moment of generating, engraving, and sealing a 5b coin, one owner alone)
